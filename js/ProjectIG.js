...
function initScreenAnd3D() {
  ...
  document.addEventListener("keydown", handleMouseDown, false);
...
Carrot = function() {
  ...
  var bodyGeom = new THREE.CubeGeometry(5, 10, 4,1);
  bodyGeom.vertices[2].y-=15;
  bodyGeom.vertices[3].y-=15;
  bodyGeom.vertices[6].y-=15;
  bodyGeom.vertices[7].y-=15;
...
function updateCarrotPosition(){
  carrot.mesh.position.y = -floorRadius + Math.sin(floorRotation+carrot.angle) * (floorRadius+25);
...
function updateObstaclePosition(){
  ...
  obstacle.mesh.position.y = -floorRadius + Math.sin(floorRotation+obstacle.angle) * (floorRadius+25);
...
function checkCollision(){
  ...
  var df = hero.mesh.position.clone().sub(floor.fir.mesh.position.clone());

  if(df.length() < collisionObstacle && obstacle.status != "flying"){
    gameOver();
  }
}
